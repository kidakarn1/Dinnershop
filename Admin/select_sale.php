...
<?php
 $spt=0+$_GET[sp];
    $sql5 ="select* from orderdns,product where orderdns.pro_id=product.pro_id order by order_id desc limit $spt,10";
...
?>
		...
		<td bgcolor="#ffffff"><center><?php echo $row5[order_status]?></center></td>
		<td bgcolor="#ffffff"></center>

		<form method="post" action="edit_orderstatus.php">

	<?php 
		if($row5[order_status]=='กำลังดำเนินการ'){
	?>
		<input type="hidden" name="order_id"value="<?php echo $row1[order_id]?>">
		<input type="submit"value="กำลังจัดส่งสินค้า"name="status">
	<?php 
	}
	?>

	<?php 
		if($row5[order_status]=='กำลังจัดส่งสินค้า'){
	?>
		<input type="hidden" name="order_id"value="<?php echo $row1[order_id]?>">
		<input type="submit"value="จัดส่งสินค้าเสร็จสิ้น"name="status">
	<?php 
	}
	?>
...
?>
<th colspan="9"><a href="bill.php?id=<?php echo $row1[order_id];?>"><font color="#ff0033"><center>ปริ้นใบเสร็จหมายเลข
